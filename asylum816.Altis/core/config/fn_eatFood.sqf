...
if([false,_food,1] call life_fnc_handleInv) then {
	switch (_food) do
	{
		...
		case "kebab": {_val = 60};
		case "donerkebab": {if (life_donator > 0) then {_val = (life_donator * (60/7))} else {_val = 10}};
	};
	...
	{
		if (random 10 < 1) then { life_hunger = 0; life_thirst = 0; player setFatigue 1; hint "You feel incredibly ill from eating this meat!" };
	};
};