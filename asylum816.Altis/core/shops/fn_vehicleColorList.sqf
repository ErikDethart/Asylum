		...
		switch(life_veh_shop) do
		{
			...
			default
			{
                ...
                if((_c_arr select _i) select 1 in _id) then
				{
					...
					if (life_donator >= _donation_req && (_lootitem_req == 0 || _lootitem_req in life_lootRewards) && (_prestige_req == 0 || _prestige_req in life_achievements) && (_shop_req == "" || _shop_req == life_veh_shop)) then
					{
